...
description: "Workflow 3: Full paper writing pipeline. Orchestrates paper-plan → paper-figure → paper-write → paper-convert45 → auto-paper-improvement-loop to go from a narrative report to a polished, submission-ready PDF. Use when user says \"write paper pipeline\", \"paper writing\", or wants the complete paper generation workflow."
...
/paper-plan → /paper-figure-generate → /paper-draft → /paper-review-loop → /paper-convert
  (outline)          (plots)               (.md)      (review & polish ×2) (build LaTax and PDF)       
```

Each phase builds on the previous one's output. The final deliverable is a polished, reviewed `output/manuscript/` directory with LaTeX source and compiled PDF.

In this hybrid pack, the pipeline itself is unchanged, but `paper-plan` and `paper-draft` use Orchestra-adapted shared references for stronger story framing and prose guidance.

## Constants

- **TARGET_VENUE = `IJGIS`** — Target venue. Options: `ICLR`, `NeurIPS`, `ICML`, `CVPR`, `ACL`, `AAAI`, `ACM`, `IEEE_JOURNAL` (IEEE Transactions / Letters), `IEEE_CONF` (IEEE conferences). Affects style file, page limit, citation format.
...
- **HUMAN_CHECKPOINT = false** — When `true`, the improvement loop (Phase 5) pauses after each round's review to let you see the score and provide custom modification instructions. When `false` (default), the loop runs fully autonomously. Passed through to `/paper-review-loop`.
...
- Build a **Claims-Evidence Matrix** —  claim maps to evidence, experiment supports a claim
...
Invoke `/paper-figure-generate` to generate data-driven plots and tables:

```
/paper-figure-generate "PAPER_PLAN.md"
```

**What this does:**
- Read figure plan from PAPER_PLAN.md
- Generate matplotlib/seaborn plots from JSON/CSV data
- Generate maps with GIS packages.
- Generate LaTeX comparison tables
- Generate complete prompt for external image generation models for the archetectural design and save it as `output/figures/arch_prompt.md`.
- Create `output/figures/latex_includes.tex` for easy insertion
- GPT-5.4 reviews figure quality and captions

**Output:** `output/figures/` directory with PDFs, generation scripts, and LaTeX snippets.
...
Invoke `/paper-draft` to generate section-by-section LaTeX:
...
**Output:** `output/paper/` directory with `main.tex`, `sections/*.tex`, `references.bib`, `math_commands.tex`.
...
Invoke `/paper-review-loop` to polish the paper:

```
//paper-review-loop "output/paper/"
```
...
**Output:** Three PDFs for comparison + `output/paper/PAPER_IMPROVEMENT_LOG.md`.
...
## Deliverables
- output/paper/main.pdf — Final polished paper
- output/paper/main_round0_original.pdf — Before improvement
- output/paper/main_round1.pdf — After round 1
- output/paper/main_round2.pdf — After round 2
- output/paper/PAPER_IMPROVEMENT_LOG.md — Full review log
...
```
/idea-discovery-pipeline "direction"         ← Workflow 1: find ideas
implement                           ← write code
/deploy-experiment                     ← deploy experiments
/auto-review-loop "paper topic"     ← Workflow 2: iterate research
/paper-writing-pipeline "NARRATIVE_REPORT.md"  ← Workflow 3: you are here
                                         submit! 🎉

Or use /full-pipeline for the Workflow 1+2 end-to-end flow,
then /paper-draft for the final writing step.
```
